...
import { AlwaysOnSampler, BatchSpanProcessor, SimpleSpanProcessor } from "@opentelemetry/sdk-trace-base";
...
var ClaudeAgentSDKInstrumentation = class extends InstrumentationBase {
	constructor() {
		super("@lmnr/claude-agent-instrumentation", version, { enabled: true });
	}
	init() {
		return new InstrumentationNodeModuleDefinition("@anthropic-ai/claude-agent-sdk", [">=0.1.0"], this.patch.bind(this), this.unpatch.bind(this));
	}
...
var Laminar = class Laminar {
	...
	*/
	...
	static getCurrentSpan(context$1) {
		const currentSpan = trace.getSpan(context$1 ?? LaminarContextManager.getContext()) ?? trace.getActiveSpan();
		if (currentSpan === void 0 || !isSpanContextValid(currentSpan.spanContext())) return;
		if (currentSpan instanceof LaminarSpan) return currentSpan;
	...
	*/
	...
	static setSpanTags(tags) {
		const currentSpan = Laminar.getCurrentSpan();
		if (currentSpan !== void 0 && isSpanContextValid(currentSpan.spanContext())) currentSpan.setTags(tags);
	}
	static addSpanTags(tags) {
		const currentSpan = Laminar.getCurrentSpan();
		if (currentSpan !== void 0 && isSpanContextValid(currentSpan.spanContext())) currentSpan.addTags(tags);
	}
	/**
	...
	* // |  | openai.chat
	*/
	static startSpan({ name, input, spanType, context: context$1, parentSpanContext, tags, userId, sessionId, metadata }) {
	...
	* // |  |  | openai.chat
	*/
	static startActiveSpan({ name, input, spanType, context: context$1, parentSpanContext, tags, userId, sessionId, metadata }) {
		return this._startSpan({
			name,
			input,
			spanType,
	...
	static _startSpan({ name, input, spanType, context: context$1, parentSpanContext, tags, userId, sessionId, metadata, activated }) {
		let entityContext = context$1 ?? LaminarContextManager.getContext();
		let parentPath;
		let parentIdsPath;
		let parentMetadata;
		let parentTraceType;
		...
		if (parentSpanContext) try {
			const laminarContext = typeof parentSpanContext === "string" ? deserializeLaminarSpanContext(parentSpanContext) : parentSpanContext;
		...
		const tracingLevelProperties = tracingLevelValue ? { [`${ASSOCIATION_PROPERTIES}.tracing_level`]: tracingLevelValue } : {};
		const metadataProperties = metadataToAttributes({
			...this.globalMetadata,
			...ctxMetadata,
			...parentMetadata,
			...metadata
	...
	*/
	static withSpan(span, fn, endOnExit) {
		...
		const ctxWithAssociationProperties = LaminarContextManager.setAssociationProperties(laminarSpan, ctx);
		const context$1 = trace.setSpan(ctxWithAssociationProperties, span);
	...
	static getLaminarSpanContext(span) {
		...
		if (currentSpan === void 0 || !isSpanContextValid(currentSpan.spanContext())) return null;
		if (!(currentSpan instanceof LaminarSpan)) currentSpan = new LaminarSpan(currentSpan);
...
var PlaywrightInstrumentation = class extends InstrumentationBase {
	...
	init() {
		return new InstrumentationNodeModuleDefinition("playwright", [">=1.0.0"], this.patch.bind(this), this.unpatch.bind(this));
	}
	...
	patchNewBrowser() {
		...
		return (original) => async function method(...args) {
			...
			for (const context$1 of browser.contexts()) {
				context$1.on("page", (page) => plugin.patchPage(page, sessionId));
				for (const page of context$1.pages()) await plugin.patchPage(page, sessionId);
			}
	...
	patchBrowserNewContext() {
		const plugin = this;
		return (original) => async function method(...args) {
			const context$1 = await original.bind(this).apply(this, args);
			const sessionId = newUUID();
			context$1.on("page", async (page) => plugin.patchPage(page, sessionId));
			for (const page of context$1.pages()) await plugin.patchPage(page, sessionId);
			return context$1;
		};
	}
...
var StagehandV2Instrumentation = class extends InstrumentationBase {
	...
	init() {
		return new InstrumentationNodeModuleDefinition("@browserbasehq/stagehand", [">=1.0.0 <3.0.0"], this.patch.bind(this), this.unpatch.bind(this));
	}
	patch(moduleExports, moduleVersion) {
...
var PuppeteerInstrumentation = class extends InstrumentationBase {
	...
	init() {
		return [new InstrumentationNodeModuleDefinition("puppeteer", [">=19.0.0"], this.patch.bind(this), this.unpatch.bind(this)), new InstrumentationNodeModuleDefinition("puppeteer-core", [">=19.0.0"], this.patch.bind(this), this.unpatch.bind(this))];
	}
	...
	patchNewBrowser() {
		...
		return (original) => async function method(...args) {
			const browser = await original.call(this, ...args);
			const sessionId = newUUID();
			for (const context$1 of browser.browserContexts()) {
				context$1.on("targetcreated", (target) => {
				...
				await Promise.all((await context$1.pages()).map((page) => plugin.patchPage(page, sessionId)));
			}
	...
	patchBrowserNewContext() {
		...
		return (original) => async function method(...args) {
			...
			context$1.on("targetchanged", (target) => {
				target.page().then((page) => {
					if (page) plugin.patchPage(page, sessionId).catch((error) => {
						logger$6.error(`Failed to patch page: ${error instanceof Error ? error.message : String(error)}`);
					});
			...
			for (const page of await context$1.pages()) await plugin.patchPage(page, sessionId);
			return context$1;
		};
	}
	async patchPage(page, sessionId) {
...
*/
function createTargetCreatedHandler(context$1, state, sessionRecordingOptions) {
	return async (event) => {
		try {
			if (event.targetInfo.type !== "page") return;
			await new Promise((resolve) => setTimeout(resolve, 200));
			for (const page of context$1.pages()) try {
				const pageId = page.mainFrameId();
				if (!state.instrumentedPageIds.has(pageId)) await injectRecorderViaCDP(page, state, context$1.conn, sessionRecordingOptions, state.bindingHandler ?? void 0);
			} catch (error) {
...
*/
function createTargetInfoChangedHandler(context$1, state, sessionRecordingOptions) {
	return async (event) => {
		if (event.targetInfo.type !== "page") return;
		...
		try {
			for (const page of context$1.pages()) if (page.targetId() === event.targetInfo.targetId) {
				await injectRecorderViaCDP(page, state, context$1.conn, sessionRecordingOptions, state.bindingHandler ?? void 0);
				break;
			}
		} catch (error) {
...
var StagehandInstrumentation = class extends InstrumentationBase {
	...
	*/
	...
	init() {
		return new InstrumentationNodeModuleDefinition("@browserbasehq/stagehand", [">=3.0.0"], this.patch.bind(this), this.unpatch.bind(this));
	}
	patch(moduleExports, moduleVersion) {
		diag.debug(`patching stagehand ${moduleVersion}`);
		const descriptor = Object.getOwnPropertyDescriptor(moduleExports, "Stagehand");
	...
	*/
	async setupSessionRecording(stagehandInstance, sessionId, traceId) {
		...
		const context$1 = stagehandInstance.context;
		if (!context$1 || !context$1.conn) {
		...
		const targetCreatedHandler = createTargetCreatedHandler(context$1, recorderState, this._sessionRecordingOptions);
		recorderState.targetCreatedHandler = targetCreatedHandler;
		context$1.conn.on("Target.targetCreated", targetCreatedHandler);
		const targetInfoChangedHandler = createTargetInfoChangedHandler(context$1, recorderState, this._sessionRecordingOptions);
		recorderState.targetInfoChangedHandler = targetInfoChangedHandler;
		context$1.conn.on("Target.targetInfoChanged", targetInfoChangedHandler);
		try {
			const pages = context$1.pages();
			let injectedCount = 0;
			for (const page of pages) try {
				if (await injectRecorderViaCDP(page, recorderState, context$1.conn, this._sessionRecordingOptions, bindingHandler) !== null) injectedCount++;
			} catch (error) {
	...
	cleanupSessionRecording(stagehandInstance) {
		...
		if (context$1 && context$1.conn) {
			if (recorderState.targetCreatedHandler) try {
				context$1.conn.off("Target.targetCreated", recorderState.targetCreatedHandler);
			} catch (error) {
			...
			if (recorderState.targetInfoChangedHandler) try {
				context$1.conn.off("Target.targetInfoChanged", recorderState.targetInfoChangedHandler);
			} catch (error) {
	...
	patchStagehandGlobalMethod(methodName, sessionId, parentSpan) {
		return (original) => async function method(...args) {
			...
			return await Laminar.withSpan(currentSpan ?? parentSpan, async () => await observe({
				...
				(trace.getSpan(LaminarContextManager.getContext()) ?? trace.getActiveSpan())?.setAttribute(TRACE_HAS_BROWSER_SESSION, true);
...
var KernelInstrumentation = class extends InstrumentationBase {
	constructor() {
		super("@lmnr/kernel-instrumentation", version, { enabled: true });
	}
	init() {
		return new InstrumentationNodeModuleDefinition("@onkernel/sdk", [">=0.7.0"], this.patch.bind(this), this.unpatch.bind(this));
	}
	...
	patchKernelAppAction() {
		return (original) => function(name, handler) {
			const wrappedHandler = async (context$1, payload) => {
...
const getStagehandInstrumentation = (playwrightInstrumentation, client, sessionRecordingOptions) => {
	try {
		const version$1 = __require("@browserbasehq/stagehand/package.json").version;
...
const initInstrumentations = (client, suppressContentTracing, sessionRecordingOptions) => {
	...
	instrumentations.push(new AzureOpenAIInstrumentation({ traceContent: !suppressContentTracing }));
	...
	instrumentations.push(new LlamaIndexInstrumentation({ traceContent: !suppressContentTracing }));
	instrumentations.push(new TogetherInstrumentation({ traceContent: !suppressContentTracing }));
	instrumentations.push(new ChromaDBInstrumentation({ traceContent: !suppressContentTracing }));
	instrumentations.push(new QdrantInstrumentation({ traceContent: !suppressContentTracing }));
	if (client) {
...
const manuallyInitInstrumentations = (client, instrumentModules, suppressContentTracing, sessionRecordingOptions) => {
	...
	if (instrumentModules?.anthropic) {
		const anthropicInstrumentation = new AnthropicInstrumentation({ traceContent: !suppressContentTracing });
		instrumentations.push(anthropicInstrumentation);
		anthropicInstrumentation.manuallyInstrument(instrumentModules.anthropic);
	}
	if (instrumentModules?.azureOpenAI) {
		const azureOpenAIInstrumentation = new AzureOpenAIInstrumentation({ traceContent: !suppressContentTracing });
	...
	if (instrumentModules?.cohere) {
		const cohereInstrumentation = new CohereInstrumentation({ traceContent: !suppressContentTracing });
	...
	if (instrumentModules?.langchain) {
		const langchainInstrumentation = new LangChainInstrumentation({ traceContent: !suppressContentTracing });
	...
	if (instrumentModules?.llamaIndex) {
		const llamaIndexInstrumentation = new LlamaIndexInstrumentation({ traceContent: !suppressContentTracing });
		instrumentations.push(llamaIndexInstrumentation);
		llamaIndexInstrumentation.manuallyInstrument(instrumentModules.llamaIndex);
	}
	if (instrumentModules?.chromadb) {
		const chromadbInstrumentation = new ChromaDBInstrumentation({ traceContent: !suppressContentTracing });
	...
	if (instrumentModules?.qdrant) {
		const qdrantInstrumentation = new QdrantInstrumentation({ traceContent: !suppressContentTracing });
	...
	if (instrumentModules?.together) {
		const togetherInstrumentation = new TogetherInstrumentation({ traceContent: !suppressContentTracing });
...
var LaminarSpanProcessor = class LaminarSpanProcessor {
	...
	*/
	...
	onStart(span, parentContext) {
		const parentPathFromAttribute = span.attributes?.[PARENT_SPAN_PATH];
		const parentIdsPathFromAttribute = span.attributes?.[PARENT_SPAN_IDS_PATH];
		...
		span.setAttribute(SPAN_SDK_VERSION, version);
		const langVersion = getLangVersion();
		if (langVersion) span.setAttribute(SPAN_LANGUAGE_VERSION, langVersion);
		const associationProperties = context.active().getValue(ASSOCIATION_PROPERTIES_KEY);
		if (associationProperties) for (const [key, value] of Object.entries(associationProperties)) if (Object.keys(ASSOCIATION_PROPERTIES_OVERRIDES).includes(key)) span.setAttribute(ASSOCIATION_PROPERTIES_OVERRIDES[key], value);
		else if (key === "tracing_level") span.setAttribute("lmnr.internal.tracing_level", value);
		else span.setAttribute(`${ASSOCIATION_PROPERTIES}.${key}`, value);
...
var LaminarTracer = class {
	...
	startSpan(name, options, context$1) {
		return new LaminarSpan(this._tracer.startSpan(name, options, context$1 || LaminarContextManager.getContext()));
	}
	startActiveSpan(name, optionsOrFn, contextOrFn, fn) {
		...
		else contextToUse = LaminarContextManager.getContext();
		const wrapped = (fn$1) => (span) => {
			const laminarSpan = new LaminarSpan(span);
			const ctxWithAssociationProperties = LaminarContextManager.setAssociationProperties(laminarSpan, contextToUse);
			...
			return LaminarContextManager.runWithIsolatedContext([...currentStack, newContext], () => fn$1(laminarSpan));
		};
...
const isGlobalContextManagerConfigured = () => {
	...
	context.with(testContext, () => {
		isContextManagerWorking = context.active().getValue(testKey) === "lmnr-test-value";
	});
...
*/
const getTracerProvider = () => tracerProvider ?? trace.getTracerProvider();
/**
* Get the tracer.
...
const initializeTracing = (options) => {
	...
	if (options.logLevel) diag.setLogger(new DiagConsoleLogger(), {
...
function observeBase({ name, associationProperties, input, ignoreInput, ignoreOutput, inputParameters, suppressTracing: shouldSuppressTracing, parentSpanContext }, fn, thisArg, ...args) {
	...
	if (shouldSuppressTracing) entityContext = suppressTracing(entityContext);
	if (parentSpanContext) try {
		const spanContext = tryToOtelSpanContext(typeof parentSpanContext === "string" ? deserializeLaminarSpanContext(parentSpanContext) : parentSpanContext);
		entityContext = trace.setSpan(entityContext, trace.wrapSpanContext(spanContext));
	} catch (e) {
	...
	return context.with(entityContext, () => getTracer().startActiveSpan(name, { attributes: associationProperties }, entityContext, async (span) => {
		if (shouldSendTraces() && !ignoreInput) try {
			...
			else if (spanInput.length === 1 && typeof spanInput[0] === "object" && !(spanInput[0] instanceof Map)) span.setAttribute(SPAN_INPUT, typeof spanInput[0] === "string" ? spanInput[0] : serialize(spanInput[0]));
			else span.setAttribute(SPAN_INPUT, serialize(spanInput.length > 0 ? spanInput : {}));
		} catch (error) {
		...
		Object.entries(associationProperties || {}).forEach(([key, value]) => {
			if (isOtelAttributeValueType(value)) span.setAttribute(key, value);
		...
		if (res instanceof Promise) return res.then((resolvedRes) => {
			try {
				if (shouldSendTraces() && !ignoreOutput) span.setAttribute(SPAN_OUTPUT, serialize(resolvedRes));
			} catch (error) {
		...
		try {
			if (shouldSendTraces() && !ignoreOutput) span.setAttribute(SPAN_OUTPUT, typeof res === "string" ? res : serialize(res));
		} catch (error) {
...
function withTracingLevel(tracingLevel, fn, ...args) {
	...
	const result = LaminarContextManager.runWithIsolatedContext([entityContext], () => fn(...args));
	const newAssociationProperties = entityContext.getValue(ASSOCIATION_PROPERTIES_KEY) ?? {};
	delete newAssociationProperties["tracing_level"];
	entityContext = entityContext.setValue(ASSOCIATION_PROPERTIES_KEY, newAssociationProperties);
	return result;
}
const buildAssociationProperties = (options) => {
	...
	if (sessionIdValue) associationProperties[SESSION_ID] = sessionIdValue;
	const userIdValue = options.userId ?? parentUserId ?? ctxAssociationProperties.userId;
	if (userIdValue) associationProperties[USER_ID] = userIdValue;
	...
	if (traceTypeValue) associationProperties[TRACE_TYPE] = traceTypeValue;
	const tracingLevelValue = parentTracingLevel ?? ctxAssociationProperties.tracingLevel;
	if (tracingLevelValue) associationProperties[`${ASSOCIATION_PROPERTIES}.tracing_level`] = tracingLevelValue;
	if (options.spanType) associationProperties[SPAN_TYPE] = options.spanType;
	if (options.tags) associationProperties[`${ASSOCIATION_PROPERTIES}.tags`] = Array.from(new Set(options.tags));
	const metadataProperties = metadataToAttributes({
		...globalMetadata,
...
var Evaluation = class {
	constructor({ data, executor, evaluators, groupName, name, metadata, config: config$1 }) {
		...
		this.progressReporter = new EvaluationReporter(config$1?.baseUrl);
		this.data = data;
		...
		this.name = name;
		if (config$1) {
			if (config$1.concurrencyLimit !== void 0 && config$1.concurrencyLimit < 1) {
				logger.warn(`concurrencyLimit must be greater than 0. Setting to default of ${DEFAULT_CONCURRENCY}`);
				this.concurrencyLimit = DEFAULT_CONCURRENCY;
			} else this.concurrencyLimit = config$1.concurrencyLimit ?? DEFAULT_CONCURRENCY;
			this.traceDisableBatch = config$1.traceDisableBatch ?? false;
			this.traceExportTimeoutMillis = config$1.traceExportTimeoutMillis;
			this.traceExportBatchSize = config$1.traceExportBatchSize ?? MAX_EXPORT_BATCH_SIZE;
		}
		if (Laminar.initialized()) {
			...
			if (config$1?.projectApiKey && config$1.projectApiKey !== Laminar.getProjectApiKey()) logger.warn("Laminar was already initialized with a different project API key. Ignoring the project API key from the evaluation config.");
			return;
		}
		const key = config$1?.projectApiKey ?? process.env.LMNR_PROJECT_API_KEY;
		...
		const url = config$1?.baseUrl ?? process?.env?.LMNR_BASE_URL ?? "https://api.lmnr.ai";
		const httpUrl = config$1?.baseHttpUrl ?? url;
		const httpPort = config$1?.httpPort ?? (httpUrl.match(/:\d{1,5}$/g) ? parseInt(httpUrl.match(/:\d{1,5}$/g)[0].slice(1)) : 443);
		...
		Laminar.initialize({
			projectApiKey: config$1?.projectApiKey,
			baseUrl: url,
			baseHttpUrl,
			httpPort,
			grpcPort: config$1?.grpcPort,
			instrumentModules: config$1?.instrumentModules,
	...
	async evaluateDatapoint(evalId, datapoint, index) {
		return observe({
			...
			const partialDatapoint = {
				...
				traceId: otelTraceIdToUUID(trace.getSpan(LaminarContextManager.getContext()).spanContext().traceId),
				executorSpanId,
				index
			};
				...
				const value = await observe({ name: evaluatorName }, async (output$1, target$1, data) => {
					if (evaluator instanceof HumanEvaluator) {
						...
						if (activeSpan) {
							...
							if (evaluator.options) activeSpan.setAttribute(HUMAN_EVALUATOR_OPTIONS, JSON.stringify(evaluator.options));
						}
						return null;
					} else {
						const activeSpan = trace.getSpan(LaminarContextManager.getContext());
						if (activeSpan) activeSpan.setAttribute(SPAN_TYPE, "EVALUATOR");
...
*/
async function evaluate({ data, executor, evaluators, groupName, name, metadata, config: config$1 }) {
