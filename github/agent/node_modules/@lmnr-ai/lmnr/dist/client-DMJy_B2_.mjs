import { C as SESSION_ID, N as TRACE_TYPE, O as SPAN_OUTPUT, P as USER_ID, T as SPAN_INPUT, _ as ASSOCIATION_PROPERTIES, c as isStringUUID, d as otelSpanIdToUUID, f as otelTraceIdToUUID, k as SPAN_PATH, l as metadataToAttributes, m as slicePayload, o as initializeLogger, u as newUUID, w as SPAN_IDS_PATH } from "./utils-ChSbRj0P.mjs";
import { createRequire } from "node:module";
import { config } from "dotenv";
import { ROOT_CONTEXT, context, createContextKey, trace } from "@opentelemetry/api";
...
const createResource = (attributes) => {
	try {
		const resources = __require("@opentelemetry/resources");
...
var LaminarSpan = class LaminarSpan {
	...
	getLaminarSpanContext() {
		...
		if (this._span.attributes) {
			spanPath = this._span.attributes[SPAN_PATH];
			spanIdsPath = this._span.attributes[SPAN_IDS_PATH];
			userId = this._span.attributes[USER_ID];
			sessionId = this._span.attributes[SESSION_ID];
			traceType = this._span.attributes[TRACE_TYPE];
			tracingLevel = this._span.attributes[`${ASSOCIATION_PROPERTIES}.tracing_level`] ?? TracingLevel.ALL;
			for (const [key, rawValue] of Object.entries(this._span.attributes)) if (key.startsWith(`${ASSOCIATION_PROPERTIES}.metadata.`)) {
				let value = rawValue;
				try {
	...
	get tags() {
		...
		return this._span.attributes[`${ASSOCIATION_PROPERTIES}.tags`] ?? [];
	}
	get laminarAssociationProperties() {
		...
		try {
			...
			for (const [key, rawValue] of Object.entries(this._span.attributes)) {
				if (!key.startsWith(`${ASSOCIATION_PROPERTIES}.`)) continue;
				let value = rawValue;
			...
			properties.tracingLevel = this._span.attributes[`${ASSOCIATION_PROPERTIES}.tracing_level`] ?? TracingLevel.ALL;
			properties.userId = this._span.attributes[USER_ID] ?? void 0;
			properties.sessionId = this._span.attributes[SESSION_ID] ?? void 0;
			properties.traceType = this._span.attributes[TRACE_TYPE] ?? void 0;
			return {
				metadata,
				...properties
			};
...
const CONTEXT_SPAN_PATH_KEY = createContextKey("span_path");
const ASSOCIATION_PROPERTIES_KEY = createContextKey("association_properties");
const CONTEXT_GLOBAL_METADATA_KEY = createContextKey("global_metadata");
var LaminarContextManager = class {
	...
	static getContext() {
		...
		for (let i = contexts.length - 1; i >= 0; i--) {
			const context$1 = contexts[i];
			const span = trace.getSpan(context$1);
			if (!span) return context$1;
			if (!span.isRecording() && span.spanContext().isRemote) return context$1;
			if (!(span instanceof LaminarSpan)) return context$1;
			if (!span.isActivated) return context$1;
			try {
				if (this._activeSpans.has(span.spanContext().spanId)) return context$1;
			} catch {
				return context$1;
			}
		}
	...
	static pushContext(context$1) {
		const newContexts = [...this.getContextStack(), context$1];
		this._asyncLocalStorage.enterWith(newContexts);
	}
	static popContext() {
		const contexts = this.getContextStack();
		if (contexts.length > 0) {
			const newContexts = contexts.slice(0, -1).filter((context$1) => {
				const span = trace.getSpan(context$1);
				if (!span) return true;
				try {
	...
	static getContextStack() {
		return this._asyncLocalStorage.getStore() || (this._inheritGlobalContext ? [context.active()] : []);
	}
	/**
	...
	*/
	...
	static setAssociationProperties(span, context$1) {
		const properties = span.laminarAssociationProperties;
		return this.setRawAssociationProperties(properties, context$1);
	}
	static setRawAssociationProperties(properties, context$1) {
		let entityContext = context$1;
...
//# sourceMappingURL=client-DMJy_B2_.mjs.map